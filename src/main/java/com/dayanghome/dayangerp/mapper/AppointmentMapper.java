...
public interface AppointmentMapper {
    ...
    @Insert({"INSERT INTO appointment(id, customerId, contactMobile, cityId, address, appointmentTime, status, createTime)" +
            "VALUES(null, #{customerId}, #{contactMobile}, #{cityId}, #{address}, #{appointmentTime}, #{status}, #{createTime})"})
    @Options(useGeneratedKeys=true, keyProperty="id")
    int insertAppointment(Appointment appointment);

    @Select({"<script>" +
            "SELECT * FROM appointment WHERE 1 = 1" +
            "<if test=\"mobile!=null\">AND contactMobile = ${mobile} </if>" +
